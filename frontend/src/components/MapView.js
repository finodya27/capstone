...
const MapView = ({ telemetryData, fireAlerts }) => {
  ...
  // Gunakan posisi drone jika ada GPS valid, kalau tidak pakai default
  ...
  // Zoom lebih dekat kalau ada GPS
            ...
            <Popup>
              <div className="text-center">
                ...
                <span className="text-sm text-gray-600">
                  GPS: {telemetryData.gps.latitude.toFixed(6)}, {telemetryData.gps.longitude.toFixed(6)}
                </span>
                <br />
                <span className="text-sm">
                  Battery: <span className={`font-semibold ${
                    telemetryData.battery > 60 ? 'text-green-600' : 
                    telemetryData.battery > 30 ? 'text-yellow-600' : 'text-red-600'
                  }`}>
                  ...
                  <div className="text-sm space-y-1">
                    <div>
                      ...
                      <span className="text-gray-800">
                        {alert.timestamp ? new Date(alert.timestamp).toLocaleString('id-ID') : "N/A"}
                      </span>
                    </div>
                    {alert.severity && (
                      <div>
                        ...
                        <span className={`inline-block px-2 py-1 rounded text-xs font-bold uppercase ${
                          alert.severity.toLowerCase() === 'critical' ? 'bg-red-500 text-white' :
                          alert.severity.toLowerCase() === 'high' ? 'bg-orange-500 text-white' :
                          alert.severity.toLowerCase() === 'medium' ? 'bg-yellow-500 text-white' :
                    ...
                    {alert.confidence && (
                      <div>
                        ...
                        <br />
                        <span className="text-gray-800">{alert.confidence}%</span>
                      </div>
                    )}
